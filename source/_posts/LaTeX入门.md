...
空格和制表符等空白字符在LaTeX中被看作相同的空白距离(space)。**多个连续**的空白字符等同于**一个空白字符**。在句首的空白距离一般会被忽略，单个空行也被认为是一个“空白距离”。
...
下面的这些字符是LATEX中的**保留字符**(reserved characters)，它们或在LATEX中有特殊的意义，或不一定存在于所有字库中。
...
LATEX命令(commands) 是大小写敏感的，有以下两种格式：
...
有些命令需要一个参数(parameter)，该参数用花括号(curly braces) { }括住并写在命令的后面。一些命令支持可选参数(optional parameters)，可选参数 可用方括号(square brackets) [ ] 括住。
